...
function handleMessage(ws, session, msg) {
  ...
  if (type === 'join') {
    ...
    if (!room) {
      // Create room - fetch player list from backend
      fetchRoomInfo(session.roomId).then((roomInfo) => {
        let existingRoom = rooms.get(session.roomId);
        if (existingRoom) {
          // Another join already created the room while we were fetching
          ...
          // Start if enough unique players are connected
        ...
        // Only start when enough unique players are connected
